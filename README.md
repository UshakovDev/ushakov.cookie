...
Если аналитика запрещена, helper безопасно возвращает `false`/`null` и не вызывает ошибок. Если debug-режим включён, шаги пропуска логируются в консоль.
...
Если helper обнаруживает `window.ym`, который не был инициализирован модулем, в консоли появится `console.warn` — это диагностическое предупреждение, а не блокировка.
