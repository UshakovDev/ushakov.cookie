...
Начиная с этапа P2.2 модуль умеет сам подключать Яндекс.Метрику и привязывать её к решению пользователя.

### Поведение gating

| Решение пользователя | Managed mode | Counter ID | Метрика |
|---|---|---|---|
| `accepted` | включён | задан | подключается |
| `rejected` | любой | любой | **не** подключается |
| решения нет | любой | любой | **не** подключается |
| любое | выключен | любой | **не** подключается |
| любое | включён | не задан | **не** подключается |

После нажатия «Согласиться» в баннере выполняется автоматическая перезагрузка страницы (только если managed mode включён, ID счётчика задан и Метрика ещё не была разрешена). На следующем хите snippet Метрики печатается сервером штатно.

При `rejected` или отсутствии решения сервер не печатает код Метрики и сетевые запросы к `mc.yandex.ru` не уходят.

### Per-site настройки

В админке (вкладка соответствующего сайта → группа «Управляемая Яндекс.Метрика»):

- управлять подключением Метрики через модуль;
- ID счётчика;
- Webvisor;
...
- e-commerce (вкл/выкл);
- имя контейнера e-commerce (по умолчанию `dataLayer`);
- debug-режим helper'а.

Для v1 поддерживается **один управляемый счётчик на сайт**.

### JS helper

Модуль кладёт в `window` безопасный helper, чтобы сайт мог отправлять цели и e-commerce, не задумываясь о текущем состоянии consent:

```js
window.ushakovCookieAnalytics.isAllowed()                  // true/false
window.ushakovCookieAnalytics.isInitialized()              // true/false
window.ushakovCookieAnalytics.init()                       // v1: возвращает текущее состояние
window.ushakovCookieAnalytics.reachGoal('order_submit', { sum: 1000 })
window.ushakovCookieAnalytics.hit('/checkout/step-2', { title: 'Шаг 2' })
window.ushakovCookieAnalytics.pushEcommerce({ ecommerce: { /* ... */ } })
window.ushakovCookieAnalytics.getEcommerceContainerName()  // 'dataLayer' или null
```

Если аналитика запрещена, helper безопасно возвращает `false`/`null` и не вызывает ошибок. Если debug-режим включён, шаги пропуска пишутся в блок диагностики в настройках модуля.

### Диагностика Метрики

В настройках каждого сайта есть блок «Диагностика Яндекс.Метрики». Он показывает:

- включён ли managed mode;
- включён ли debug-режим;
- валиден ли ID счётчика;
- последние debug-события helper'а.

Логи пишутся только при включённом debug-режиме. Модуль сохраняет технические статусы и причины пропуска действий, но не пишет полный URL, payload e-commerce и персональные данные.

Диагностические события хранятся в таблице модуля `b_ushakov_cookie_debug_log`. Для каждого сайта сохраняются только последние 500 событий, а в настройках показываются последние 100.

### Ограничение: внешняя Метрика

Модуль **не пытается** автоматически удалять Яндекс.Метрику, если она уже подключена:

- через шаблон сайта;
- через Aspro / другое готовое решение;
- через сторонний модуль;
- через кастомный код в `init.php` или `<head>` шаблона.

Полностью поддерживаемый сценарий v1:

1. Внешнее подключение Метрики отключается (в шаблоне / другом модуле / Aspro);
2. ID счётчика указывается в настройках `ushakov.cookie`;
3. Включается «Управлять подключением Яндекс.Метрики через модуль».

Если helper обнаруживает `window.ym`, который не был инициализирован модулем, он запишет диагностическое предупреждение в лог модуля. Это предупреждение, а не блокировка.

### Что намеренно оставлено на следующие этапы

- кнопка «Настроить»;
- категории cookies;
- versioning согласия и повторный запрос по версии;
- несколько счётчиков на сайт;
- автоматическое вырезание внешней Метрики;
- управление другими провайдерами аналитики (Google Tag Manager, VK Pixel и т. п.);
- полноценный CMP-уровень.
...
- [x] **Managed Yandex Metrica gating v1**: per-site настройки, server-side gating, JS helper
- [ ] Кнопка «Настройки»
